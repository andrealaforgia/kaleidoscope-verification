# S09 — double-init-while-guard-alive-errors
...
A second spark::init while the first guard is still alive returns Err(GlobalAlreadyInitialised).

## Source

- Inter-session feed (other claude session, 2026-05-06): item **S9**.
...
- Method: driven by the **spark-consumer** fixture under
  ...
  runs the scenario `s09-double-init` and asserts the outcome line.
  S09 is an init-side check; no OTLP traffic is generated, no
