#!/usr/bin/env bash
# S08 — driven by the spark-consumer fixture, scenario
# `s08-malformed-endpoint`. Init-side check only, no aperture required.
set -euo pipefail
EVIDENCE_DIR="$1"
...
echo "step 2: run scenario s08-malformed-endpoint"
docker run --rm \
    kaleidoscope-expectations/spark-consumer:under-test \
    --scenario s08-malformed-endpoint \
...
if ! grep -qE '^scenario=s08-malformed-endpoint result=ok detail=InvalidEndpoint' "$EVIDENCE_DIR/consumer.stdout.txt"; then
    echo "consumer outcome line did not match expected pattern" >&2
    exit 1
fi

echo "OK — spark::init returned InvalidEndpoint on a malformed URL"
