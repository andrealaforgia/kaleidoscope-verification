...
A malformed endpoint URL passed to with_endpoint(...) causes spark::init to return Err(InvalidEndpoint { endpoint, reason }).

## Source

- Inter-session feed (other claude session, 2026-05-06): item **S8**.
...
- Method: driven by the **spark-consumer** fixture under
  ...
  runs the scenario `s08-malformed-endpoint` and asserts the outcome line.
  S08 is an init-side check; no OTLP traffic is generated, no
