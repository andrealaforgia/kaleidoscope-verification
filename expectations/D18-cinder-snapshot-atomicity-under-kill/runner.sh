#!/usr/bin/env bash
# D18 — cinder SNAPSHOT ATOMICITY under a mid-snapshot process kill
...
# rationale. cinder-crash-target --seed-then-loop-snapshot seeds one
...
exec "$HARNESS_DIR/assert-snapshot-atomicity.sh" "$1" D18 cinder cinder-crash-target
