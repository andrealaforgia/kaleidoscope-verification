# D13 — sluice-fsync-refusal-lying-substrate

## Surface

`sluice-crash-target --probe-lying`. Per-store instance of the D08
WAL-fsync REFUSAL pattern for the Sluice (buffer) store. Durability set (#17).

## Behaviour

Given the sluice composition root is driven against a substrate that
lies about durability (a `LyingFsyncBackend` that acks an fsync which
never reached stable storage)
When `sluice-crash-target --probe-lying` runs
