# D10 — ray-fsync-refusal-lying-substrate

## Surface

`ray-crash-target --probe-lying`. Per-store instance of the D08
WAL-fsync REFUSAL pattern for the Ray (traces) span store. Durability set (#17).

## Behaviour

Given the ray composition root is driven against a substrate that
lies about durability (a `LyingFsyncBackend` that acks an fsync which
never reached stable storage)
When `ray-crash-target --probe-lying` runs
