# S07 — missing-tenant-id-when-required-errors
...
With require_tenant_id() but no with_tenant_id, spark::init returns Err(MissingRequiredAttribute { name: "tenant.id" }).

## Source

- Inter-session feed (other claude session, 2026-05-06): item **S7**.
...
- Method: driven by the **spark-consumer** fixture under
  ...
  runs the scenario `s07-missing-tenant-id` and asserts the outcome line.
  S07 is an init-side check; no OTLP traffic is generated, no
