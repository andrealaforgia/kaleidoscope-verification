# D11 — strata-fsync-refusal-lying-substrate

## Surface

`strata-crash-target --probe-lying`. Per-store instance of the D08
WAL-fsync REFUSAL pattern for the Strata (profiles) store. Durability set (#17).

## Behaviour

Given the strata composition root is driven against a substrate that
lies about durability (a `LyingFsyncBackend` that acks an fsync which
never reached stable storage)
When `strata-crash-target --probe-lying` runs
