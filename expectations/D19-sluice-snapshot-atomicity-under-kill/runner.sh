#!/usr/bin/env bash
# D19 — sluice SNAPSHOT ATOMICITY under a mid-snapshot process kill
...
# rationale. sluice-crash-target --seed-then-loop-snapshot seeds one
...
exec "$HARNESS_DIR/assert-snapshot-atomicity.sh" "$1" D19 sluice sluice-crash-target
