#!/usr/bin/env bash
# D20 — beacon SNAPSHOT ATOMICITY under a mid-snapshot process kill
...
# rationale. beacon-crash-target --seed-then-loop-snapshot seeds one
...
exec "$HARNESS_DIR/assert-snapshot-atomicity.sh" "$1" D20 beacon beacon-crash-target
