# D20 — beacon-snapshot-atomicity-under-kill

## Surface

`beacon-crash-target --seed-then-loop-snapshot`. Per-store instance of
the D09 SNAPSHOT ATOMICITY pattern for the Beacon (alerting) store. Durability
set (#17). Black-box ground for
[issue 007](../../issues/007-non-atomic-snapshot-write-can-brick-the-store.md).

## Behaviour

Given an alerting rule-state seeded as one acked datum (the crash-target prints
...
- Last verified: 2026-06-05 UTC at HEAD (`a812193`). GREEN — see
  `evidence/D20.stdout.txt` for `snapshot_whole`, `snapshot_present`,
