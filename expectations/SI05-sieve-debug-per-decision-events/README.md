# SI05 — sieve-debug-per-decision-events
...
Each sampling decision emits one DEBUG-level structured tracing event naming the trace_id, the decision (keep/drop), and the rate that produced it. The event is observable on the host process's stderr when RUST_LOG enables sieve at DEBUG.
