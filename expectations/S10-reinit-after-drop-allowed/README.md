...
After the first guard is dropped, a subsequent spark::init returns Ok(...) again — sequential init→drop→init is permitted at v0.6.
...
- Method: driven by the **spark-consumer** fixture under
  ...
  runs the scenario `s10-reinit-after-drop` and asserts the outcome line.
  S10 is an init-side check; no OTLP traffic is generated, no
