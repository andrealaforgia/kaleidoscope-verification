# A07 — grpc-rejects-malformed-bytes
...
Malformed gRPC bytes are rejected with INVALID_ARGUMENT and a recognisable Rule discriminator (EmptyInput, ProtobufDecode, SignalMismatch).
...
- Inter-session feed (other claude session, 2026-05-06): item **A7**.
...
```
harness/run-expectation.sh A07
```
