# D17 — strata-snapshot-atomicity-under-kill

## Surface

`strata-crash-target --seed-then-loop-snapshot`. Per-store instance of
the D09 SNAPSHOT ATOMICITY pattern for the Strata (profiles) store. Durability
set (#17). Black-box ground for
[issue 007](../../issues/007-non-atomic-snapshot-write-can-brick-the-store.md).

## Behaviour

Given a profile seeded as one acked datum (the crash-target prints
...
- Last verified: 2026-06-05 UTC at HEAD (`a812193`). GREEN — see
  `evidence/D17.stdout.txt` for `snapshot_whole`, `snapshot_present`,
  `record_in_snapshot`/`record_in_wal`, `tmp_at_kill`.
- Method: `harness/assert-snapshot-atomicity.sh` →
  `harness/run-crash-target.sh` builds `strata-crash-target` from the
...
- store-fsync-durability-v0; `crates/strata/src/bin/strata_crash_target.rs`
