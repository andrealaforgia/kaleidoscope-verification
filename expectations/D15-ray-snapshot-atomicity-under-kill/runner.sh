#!/usr/bin/env bash
# D15 — ray SNAPSHOT ATOMICITY under a mid-snapshot process kill
...
# rationale. ray-crash-target --seed-then-loop-snapshot seeds one
...
exec "$HARNESS_DIR/assert-snapshot-atomicity.sh" "$1" D15 ray ray-crash-target
