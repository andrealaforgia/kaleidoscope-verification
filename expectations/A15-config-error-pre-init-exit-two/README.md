...
A bad config at startup causes stderr to print "aperture: config error: ..." (the only pre-tracing stderr write path) and the process to exit with code 2.
...
```
harness/run-expectation.sh A15
```
