# A14 — drain-deadline-exceeded-exit-one
...
If the drain deadline expires with requests still in flight, the process exits with code 1 and stderr emits event=drain_deadline_exceeded with a dropped count (per N5, the value is currently the literal "unknown"; the prefix is the contract).
...
- Inter-session feed (other claude session, 2026-05-06): item **A14**.
...
```
harness/run-expectation.sh A14
```
