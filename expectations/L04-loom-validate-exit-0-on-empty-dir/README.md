# L04 — loom-validate-exit-0-on-empty-dir
...
`loom validate --rules <DIR>` exits 0 when DIR exists and contains no rule files. Empty-directory case is treated as success: a fresh team that has not yet authored any rules is not blocked.
...
- [`evidence/L04.stdout.txt`](evidence/L04.stdout.txt) — combined stdout from the inline script (fixture + loom invocation).
- [`evidence/L04.stderr.txt`](evidence/L04.stderr.txt) — cargo build noise + loom's own stderr.
