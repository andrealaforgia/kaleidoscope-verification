# D12 — cinder-fsync-refusal-lying-substrate

## Surface

`cinder-crash-target --probe-lying`. Per-store instance of the D08
WAL-fsync REFUSAL pattern for the Cinder (tier metadata) store. Durability set (#17).

## Behaviour

Given the cinder composition root is driven against a substrate that
lies about durability (a `LyingFsyncBackend` that acks an fsync which
never reached stable storage)
When `cinder-crash-target --probe-lying` runs
