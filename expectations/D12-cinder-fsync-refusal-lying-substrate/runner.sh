#!/usr/bin/env bash
# D12 — cinder WAL-fsync REFUSAL on a lying substrate (black-box).
# Per-store instance of the D08 pattern (see
# harness/assert-probe-lying-refusal.sh and D08's README for the full
# rationale). cinder-crash-target --probe-lying must refuse a lying
...
exec "$HARNESS_DIR/assert-probe-lying-refusal.sh" "$1" D12 cinder cinder-crash-target
