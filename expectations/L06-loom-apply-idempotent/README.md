...
`loom apply --from <src> --to <dest>` is idempotent: running apply twice against the same source produces no further changes on the second run; both invocations exit 0. KPI 3: an operator can run apply as often as they like without diverging the destination state.

## Source

- External contract anchor:
  [`docs/feature/loom-v0/slices/slice-03-apply.md`](https://github.com/andrealaforgia/kaleidoscope/blob/HEAD/docs/feature/loom-v0/slices/slice-03-apply.md).
...
- [`evidence/L06.stdout.txt`](evidence/L06.stdout.txt) — combined stdout from the inline script (fixture + loom invocation).
- [`evidence/L06.stderr.txt`](evidence/L06.stderr.txt) — cargo build noise + loom's own stderr.
