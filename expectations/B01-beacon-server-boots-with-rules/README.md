...
`beacon-server --rules <DIR> --backend <URL>` boots, loads every TOML rule under DIR (recursive), spawns one Tokio task per rule, and stays running until SIGINT or SIGTERM. Malformed rules are surfaced as diagnostics on stderr without preventing the well-formed rules from being scheduled.
