...
| `satisfied` | 77 (A 14 + S 12 + E 4 + X 14 + L 6 + K 11 + Q 2 + G 2 + EG 1 + LQ 6 + TQ 4 + D 1) |
...
| Durability (kill-9-mid-write per pillar) | acked data survives a hard process kill (operator) | **D** | 1 (D01 satisfied) | D01 = a log acked by the gateway survives a gateway SIGKILL (exit 137, no graceful flush) and is recovered from the Lumen WAL on reopen, queryable via log-query-api. Proves the WAL-flush-before-ack contract (`append_wal` flushes to the kernel before `ingest` returns); scoped to PROCESS-kill durability, NOT OS-crash/power-loss (no fsync). D02 (Pulse via query-api) + D03 (Ray via trace-query-api) are the natural next batch. See `known-gaps.md` #17. |
