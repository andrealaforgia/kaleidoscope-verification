...
| `satisfied` | 149 (A 18 + S 12 + E 0 + X 14 + L 6 + K 33 + Q 10 + G 6 + EG 3 + LQ 10 + TQ 7 + D 20 + B 10) |
...
| End-to-end via kaleidoscope-gateway | OTLP → pillars → query-api round-trip (operator) | **EG** | 3 (EG01-EG03 satisfied) | EG01 verifies the integration thesis under contract: telemetrygen → gateway :4318 (OTLP/HTTP) → Pulse store → query-api :9090 → matrix response. EG02 = the OTLP/gRPC counterpart (gateway :4317, distinct tonic receiver) → Pulse → query-api, same `status=success`/`__name__=gen` result, so both transports round-trip end to end. **EG03** = two services on metric gen kept as two distinct series identified by service.name (UC-GWMET-003 series identity). EG04+ (further e2e) are the natural next batch. See `known-gaps.md` N18 + N19. |
