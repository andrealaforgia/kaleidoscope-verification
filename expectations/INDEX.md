...
| `satisfied` | 80 (A 14 + S 12 + E 4 + X 14 + L 6 + K 11 + Q 2 + G 2 + EG 1 + LQ 6 + TQ 4 + D 4) |
...
(`partial`; READ tier FIXED at `2663eb5` — Q01 tightened, LQ06/TQ04 added; `kaleidoscope-gateway` still has no subscriber, G01 unchanged, flagged to the implementer).
[006 — torn WAL tail blocks recovery of intact records](../issues/006-torn-wal-tail-blocks-recovery-of-intact-records.md)
(`open`; a torn trailing WAL line — the residue of an abrupt death — makes the store fail closed (SAFE, no corrupt data) but blocks ALL intact prior records; no torn-tail truncation. Found via D04, flagged to the implementer as bug-vs-v1-limitation).
Closed:
