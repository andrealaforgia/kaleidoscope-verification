...
| `satisfied` | 147 (A 18 + S 12 + E 0 + X 14 + L 6 + K 33 + Q 10 + G 6 + EG 2 + LQ 10 + TQ 6 + D 20 + B 10) |
...
| End-to-end via kaleidoscope-gateway | OTLP → pillars → query-api round-trip (operator) | **EG** | 2 (EG01/EG02 satisfied) | EG01 verifies the integration thesis under contract: telemetrygen → gateway :4318 (OTLP/HTTP) → Pulse store → query-api :9090 → matrix response. EG02 = the OTLP/gRPC counterpart (gateway :4317, distinct tonic receiver) → Pulse → query-api, same `status=success`/`__name__=gen` result, so both transports round-trip end to end. EG03-EG05 (logs via log-query-api, traces, multi-tenant isolation, durability mid-write) are the natural next batch. See `known-gaps.md` N18 + N19. |
