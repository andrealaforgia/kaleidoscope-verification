...
| `satisfied` | 152 (A 18 + S 12 + E 0 + X 14 + L 6 + K 34 + Q 10 + G 7 + EG 4 + LQ 10 + TQ 7 + D 20 + B 10) |
...
| End-to-end via kaleidoscope-gateway | OTLP → pillars → query-api round-trip (operator) | **EG** | 4 (EG01-EG04 satisfied) | EG01 verifies the integration thesis under contract: telemetrygen → gateway :4318 (OTLP/HTTP) → Pulse store → query-api :9090 → matrix response. EG02 = the OTLP/gRPC counterpart (gateway :4317, distinct tonic receiver) → Pulse → query-api, same `status=success`/`__name__=gen` result, so both transports round-trip end to end. **EG03** = two services on metric gen kept as two distinct series identified by service.name (UC-GWMET-003 series identity). **EG04** = an explicit resource tenant.id=globex routes the record to globex, overriding the default tenant acme (UC-GWTEN-001). Further e2e (UC-LOOP-004 three-pillars-one-stream, UC-GWTEN-003 fail-closed) are the natural next batch. See `known-gaps.md` N18 + N19. |
