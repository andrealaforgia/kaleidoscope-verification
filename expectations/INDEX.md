...
| `satisfied` | 78 (A 14 + S 12 + E 4 + X 14 + L 6 + K 11 + Q 2 + G 2 + EG 1 + LQ 6 + TQ 4 + D 2) |
...
Last index refresh: 2026-06-01, observed HEAD `0043386` (verifier+channel loop tick). Idle-tick backlog: opened the **durability set** (#17) with **D01** — an acked log survives a gateway SIGKILL (exit 137, no graceful flush) and is recovered from the Lumen WAL on reopen (6/6 survivors). Proves the WAL-flush-before-ack contract; scoped honestly to process-kill, not power-loss. Kaleidoscope HEAD advanced `2663eb5..0043386` but only docs + `cli-unknown-flag-rejection-v0` DISCUSS (K11 rebuild not yet delivered), nothing observable.
...
Open issues:
[005 — read/gateway tracing subscriber](../issues/005-query-api-tracing-subscriber-missing-health-events-dropped.md)
(`partial`; READ tier (query-api/log-query-api/trace-query-api) FIXED at `2663eb5` via `read-api-tracing-subscriber-v0` — Q01 tightened, LQ06/TQ04 added; `kaleidoscope-gateway` still has no subscriber, G01 unchanged, flagged to the implementer).
Closed:
...
| `crates/log-query-api` binary | `/api/v1/logs` over Lumen (operator) | **LQ** | 6 (LQ01-LQ06 satisfied) | log-query-api has a binary target + axum listener at HEAD (`crates/log-query-api/src/main.rs`). The project ships no Dockerfile, so the catalogue authors its own (`harness/Dockerfile.log-query-api`) and stands the binary up. LQ01 = `body_contains`/`body_regex` mutual-exclusion 400 (ADR-0056 DD4). LQ02 = body_contains round-trip gateway → Lumen → log-query-api (ADR-0055, feat `1bfa609`). LQ03 = body_regex round-trip with regex semantics proven (ADR-0056, feat `6cecd63`). LQ04 = `limit`/`offset` pagination with cap-then-slice + the `invalid limit` 400 (ADR-0057, feat `47fc5ef`). LQ05 = `min_severity` floor filters across the durable boundary (ADR-0052, feat `e281fca`; closes N23). An LQ result-cap (>`MAX_RESULT_ROWS`) and fails-closed-no-tenant are the natural next batch. See `known-gaps.md` N16. |
...
| Durability (kill-9-mid-write per pillar) | acked data survives a hard process kill (operator) | **D** | 2 (D01, D02 satisfied) | D01 = a log acked by the gateway survives a gateway SIGKILL (exit 137) and is recovered from the Lumen WAL on reopen via log-query-api. Lumen's `append_wal` flushes to the kernel before `ingest` returns → PROCESS-kill durable (NOT power-loss; no fsync). D02 = the Pulse analogue (metric, query-api): Pulse's `append_wal` additionally `fsync`s (`sync_all`, ADR-0049 §4) before the ack, so it is power-loss durable too — though the harness only black-box demonstrates kill-9 survival. D03 (Ray via trace-query-api) is the natural next. See `known-gaps.md` #17. |
